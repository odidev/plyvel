...
docker-build-env_aarch64:
	docker build -t plyvel-build-arm64 -f Dockerfile_aarch64 .
...
release_aarch64: docker-build-env_aarch64
	CIBW_BUILD='cp3*-manylinux_aarch64' \
	CIBW_SKIP='cp35-manylinux_aarch64' \
	CIBW_MANYLINUX_AARCH64_IMAGE=plyvel-build-arm64 \
	CIBW_BEFORE_BUILD=scripts/cibuildwheel-before-build.sh \
	CIBW_PLATFORM=linux \
	cibuildwheel --output-dir dist
